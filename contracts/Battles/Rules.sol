...
library Rules {
    ...
    function validForceMove(
        ...
        _fromState.requireSignature(_signatures[0]);
        _toState.requireSignature(_signatures[1]);
    ...
    function validConclusionProof(
        ...
        // states must be signed by the appropriate participant
        _fromState.requireSignature(_signature[0]);
        _toState.requireSignature(_signature[1]);

        // first move must be a concluded state (transition rules will ensure this for the other states)
        // As they are only 2 participants, having 2 concluded signed states validates for all parties
        require(_fromState.isConclude(), "fromState must be Conclude");
